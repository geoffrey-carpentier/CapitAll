...
- En tant qu'utilisateur connecté, je veux ajouter un actif que je souhaite suivre (crypto, devise, métal ou action, symbole, nom), afin de commencer à enregistrer mes transactions dessus. Contrainte de validation : impossible d'ajouter deux fois le même symbole (unicité utilisateur/symbole), type restreint aux quatre valeurs autorisées.
...
- En tant qu'utilisateur connecté, je veux consulter le détail d'un actif (historique des transactions, PRU, plus-value latente, plus-value réalisée, cours courant), afin de suivre sa performance individuelle.
...
- En tant qu'utilisateur connecté, je veux basculer l'affichage de mon tableau de bord entre euro et dollar (D43), afin de lire mon patrimoine dans l'une ou l'autre devise. Contrainte : la bascule ne touche que l'affichage, les montants restent calculés et stockés en euro, la conversion applique le taux EUR/USD courant.
